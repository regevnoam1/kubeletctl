...
func LookupPod(pid int, executable string, podInfo *podList, tw table.Writer) (*ID, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to lookup container ID for PID %d: %v", pid, err)
	}

	if len(containerIDs) == 0 {
		fmt.Printf("No container IDs found for PID %d\n", pid)
		return nil, nil
	}
	...
	for _, item := range podInfo.Items {
		for _, status := range item.Status.ContainerStatuses {
			for _, extractedID := range containerIDs {
				...
				//fmt.Printf("Comparing formatted ID: %s with Pod ContainerID: %s\n", formattedContainerID, status.ContainerID)

				if status.ContainerID == formattedContainerID {
					fmt.Printf("Match found for PID %d: Pod=%s, Namespace=%s, Container=%s\n",
						pid, item.Metadata.Name, item.Metadata.Namespace, status.Name)
	...
	fmt.Printf("No matching container found for PID %d and extracted IDs %v\n", pid, containerIDs)
	return nil, nil
}
...
func LookupContainerID(pid int) ([]string, error) {
	...
	for scanner.Scan() {
		line := scanner.Text()
		//fmt.Printf("Debug: /proc/%d/cgroup line: %s\n", pid, line)
